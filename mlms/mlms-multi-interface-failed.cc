...
int main (int argc, char *argv[])
{
	...
	cmd.AddValue ("configurationFileName", "Configuraition JSON filename", configurationFileName);
	cmd.Parse (argc,argv);
	...
	while(network_itr != configuration.networkMap.end()) {
		...
		while(link_itr != bandToLinkMap.end()) {
			...
			std::cout << "AP uses " << Ipv4Address(link_itr->second.AP_IP) << " for " << band << "\n";
			...
			if( bandToStaContainerMap.find(band) != bandToStaContainerMap.end() ) {
				...
				std::cout << "ssid: " << ssid << ", band: " << band << ", Sta: " << Sta_ContainerInfo.nNodes << "\n";
				...
				for(int i=0; i < Sta_ContainerInfo.nodeContainer.GetN(); i++) {
					Ptr<Node> destNode = Sta_ContainerInfo.nodeContainer.Get(i);
					Ptr<Ipv4> ipv4 = destNode->GetObject<Ipv4>();
					std::cout << "Sta IP: " << ipv4->GetAddress(1, 0).GetAddress() << "\n";
				}
			}
	...
	while( list_itr != applicationList.end() ) {
		std::cout << "Application from " << list_itr->ssid << " ("  << list_itr->srcType << ", " << list_itr->srcIndex << ") to (" << list_itr->destType << ", " << list_itr->destIndex << ", port: " << list_itr->destPort << ") " << list_itr->band << std::endl;
		...
		serverAppKeyStream << list_itr->ssid << "/Ap/" << list_itr->destPort << "/" << list_itr->band;
		std::string serverAppKey = serverAppKeyStream.str();
		if( serverAppContainerMap.find(serverAppKey) == serverAppContainerMap.end() ) {
			uint32_t local = configuration.networkMap.find(list_itr->ssid)->second.bandToLinkMap.find(list_itr->band)->second.AP_IP;

			UdpServerMHelper udpServer(0, list_itr->destPort);
			...
			std::cout << serverAppKey << " New server app at AP added\n";
		}
		...
		if(configuration.Sta_ContainerMap.find(list_itr->ssid) != configuration.Sta_ContainerMap.end()) {
			...
			//std::map<std::string, NodeContainer_t>::iterator band_itr = Sta_ContainerMap_itr->second.begin();
			if(bandToStaContainerMap.find(list_itr->band) != bandToStaContainerMap.end() ) {
				std::cout << "working on " << list_itr->band << "\n";
				...
				for(uint32_t temp = 0; temp < staContainerInfo.nodeContainer.GetN() ; temp++) {
					std::stringstream serverAppKeyStream;
					serverAppKeyStream << list_itr->ssid << "/Sta/" << temp << "/" << list_itr->destPort << "/" << list_itr->band;
					std::string serverAppKey = serverAppKeyStream.str();
					if ( serverAppContainerMap.find(serverAppKey) == serverAppContainerMap.end() ) {
						UdpServerMHelper udpServer(0, list_itr->destPort);
						...
						serverApp.Stop(Seconds (simulationTime + 1));
						serverAppContainerMap.insert(std::make_pair(serverAppKey, serverApp));
						std::cout << serverAppKey << " New server app added\n";
					}
				}
			}
		}
		...
		UdpClientHelper udpClient(destIP, list_itr->destPort);

		udpClient.SetAttribute ("MaxPackets", UintegerValue (4294967295u));
		udpClient.SetAttribute ("Interval", TimeValue (Time ("0.00001"))); //packets/s
		udpClient.SetAttribute ("PacketSize", UintegerValue (payloadSize)); //bytes
	...
	while(itr != serverAppContainerMap.end()) {
		...
		rxBytes = payloadSize * DynamicCast<UdpServerM> (itr->second.Get(0))->GetReceived ();
		if( rxBytes > 0 ) {
			nLost = DynamicCast<UdpServerM> (itr->second.Get(0))->GetLost();
